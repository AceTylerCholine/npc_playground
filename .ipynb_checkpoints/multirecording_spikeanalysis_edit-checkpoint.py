...
class SpikeAnalysis_MultiRecording:
    ...
    def generate_event_spiketrain_df(self, baseline=10, equalize=10):
        data_rows = []
        max_pre_event_len = 0
        max_event_len = 0
        for recording_name, recording in self.ephyscollection.collection.items():
          for event_name in recording.event_dict.keys():
            # Access raw spiketrains from recording object (assuming it's in unit_spiketrains)
            unit_spiketrains = recording.unit_spiketrains
    
            # Calculate pre-event and event window indices based on timebins
            pre_event_window = pre_event_window(event, baseline, 0)
            pre_event_start, pre_event_end = int(pre_event_window[0] / self.timebin), int(pre_event_window[1] / self.timebin)
            event_start, event_end = int(event[0] / self.timebin), int((event[0] + equalize * 1000) / self.timebin)
    
            event_spiketrains = {unit: unit_spiketrains[unit][event_start:event_end] for unit in unit_spiketrains.keys()}
            pre_event_spiketrains = {unit: unit_spiketrains[unit][pre_event_start:pre_event_end] for unit in unit_spiketrains.keys() if unit in recording.freq_dict and recording.freq_dict[unit] > self.ignore_freq}
            if event_spiketrains and pre_event_spiketrains:
              for unit in event_spiketrains.keys():
                if unit in pre_event_spiketrains:
                  for i, (event_arr, pre_event_arr) in enumerate(zip(event_spiketrains[unit], pre_event_spiketrains[unit])):
                    event_data = list(pre_event_arr) + list(event_arr)
                    row = [recording_name, event_name, i + 1, unit] + event_data
                    data_rows.append(row)
                    max_pre_event_len = max(max_pre_event_len, len(pre_event_arr))
