...
class SpikeAnalysis_MultiRecording:
    ...
    def create_spiketrain_df(self):
        ...
        for recording_name, recording in self.ephyscollection.collection.items():
            for event_name in recording.event_dict.keys():
                event_timebin_idx = np.ceil(recording.event_dict[event_name] / self.timebin).astype(int)
                for unit in recording.unit_spiketrains:
                    for i in range(len(event_timebin_idx)):
                        start_idx = event_timebin_idx[i][0]
                        end_idx = event_timebin_idx[i][1]
                        if end_idx <= len(recording.unit_spiketrains[unit]):
                            spiketrain_slice = recording.unit_spiketrains[unit][start_idx:end_idx-1]
                        else:
                            spiketrain_slice = []
                        spiketrain_slice = np.array(spiketrain_slice)
