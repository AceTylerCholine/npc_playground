...
class SpikeAnalysis_MultiRecording:
    ...
    def create_spiketrain_df(self, pre_window=100, post_window=200):
        """
        This function creates a DataFrame containing spiketrains around events for all recordings and units in the EphysRecordingCollection.
        
        Args:
            pre_window (int, optional): Pre-event window size in milliseconds. Defaults to 100.
            post_window (int, optional): Post-event window size in milliseconds. Defaults to 200.
            
        Returns:
            pandas.DataFrame: A DataFrame with columns for recording name, event name, event number, unit number, and spiketrain timebins.
        """
        data = []
        for recording_name, recording in self.ephyscollection.collection.items():
            for event_name in recording.event_dict.keys():
                rounded_array = np.ceil(recording.event_dict[event_name] / self.timebin).astype(int)
                for unit in recording.unit_spiketrains:
                    for i in range(len(rounded_array)):
                        # Calculate pre-event window indices
                        pre_event_start = int(rounded_array[i][0] - (pre_window / 1000) / self.timebin)
                        pre_event_end = rounded_array[i][0]
                        # Calculate event window indices
                        event_start = rounded_array[i][0]
                        event_end = int(rounded_array[i][1] + (post_window / 1000) / self.timebin)
                        # Ensure indices are within bounds
                        pre_event_start = max(0, pre_event_start)
                        event_end = min(len(recording.unit_spiketrains[unit]), event_end)
                        spiketrain_pre_event = recording.unit_spiketrains[unit][pre_event_start:pre_event_end]
                        spiketrain_event = recording.unit_spiketrains[unit][event_start:event_end]
                        # Combine pre-event and event spiketrains
                        spiketrain_slice = np.concatenate([spiketrain_pre_event, spiketrain_event])
                        row_data = [recording_name, event_name, i+1, unit] + spiketrain_slice.tolist()
                        data.append(row_data)
        # Define column names based on window sizes
        pre_event_bins = [f"Pre-event bin_{i+1}" for i in range(int(pre_window / self.timebin))]
        event_bins = [f"Event bin_{i+1}" for i in range(int(post_window / self.timebin))]
        columns = ["Recording", "Event name", "Event number", "Unit number"] + pre_event_bins + event_bins
