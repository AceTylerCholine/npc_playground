...
%trials with a sliding window of 5 ms from 0-150ms after laser onset. It uses the trials from decoder vars
clear all
close all
...
list=dir;
 ...
 %
for j=[13]; %folder number corresponds to animal folder
...
for trialnum=1:length(laseron)
    ...
    zeroedraster=t(1:end)-laseron(trialnum);
     yy=zeroedraster(zeroedraster<0 & zeroedraster>-bin);%%
